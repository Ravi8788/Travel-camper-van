...
export function RealBookingPage() {
  ...
  useEffect(() => {
    ...
    const verifyCustomer = async () => {
      ...
      if (!user || profile?.role !== "customer") {
        const next = `/book${searchParams.toString() ? `?${searchParams.toString()}` : ""}`;
  ...
  if (checkingAuth) return <Container className="py-16"><Card className="mx-auto max-w-xl"><CardContent className="p-8 text-center text-slate-500"><LoaderCircle className="mx-auto h-8 w-8 animate-spin" /><p className="mt-4">Checking your customer account...</p></CardContent></Card></Container>;
